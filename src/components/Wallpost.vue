<template>
    <div class="container">  
      <h1 class=" mt-2 mx-auto">You are in {{ name }} wall </h1>
...
//to get data from the users and post
const fetchData = async () => {
  try {
    
    const route = useRoute()
    const user_id = route.params.uid  
    
    const qPost = query(postsRef, where('userId', '==', user_id))
    const qUser = query(userRef, where('uid', '==', user_id))
